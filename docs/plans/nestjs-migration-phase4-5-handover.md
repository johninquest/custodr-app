...
### 2.4 Housekeeping (small, do alongside)

- **`.env.example` has an uncommitted edit** — `DB_PATH` commented out,
  `DATABASE_URL` added. Review and commit. It still says "Database (SQLite…)"
  in the header comment, which is now wrong.
- **`apps/api-nest/tsconfig.build.tsbuildinfo` is tracked** — a build
  artifact. Add `*.tsbuildinfo` to `.gitignore` and `git rm --cached` it.
- **Turborepo env passthrough** — `turbo.json` `globalPassThroughEnv` lacks
  `DATABASE_URL`, `PORT`, `FRONTEND_URL`, `NODE_ENV`, `LOG_LEVEL`,
  `FIREBASE_PROJECT_ID`, `FIREBASE_CREDENTIALS_PATH`, `MAILJET_*`. Tasks
  won't see them.
- **Port conflict** — `apps/api` (Go) and `apps/api-nest` both default to
  8080, so `npm run dev` starts both and one fails. Add
  `"dev:nest": "turbo run dev --filter=api-nest --filter=web"`. Do **not**
  repoint `npm run dev` until Go is archived in Phase 5.
- **Postgres publishes no host port** — add `5432:5432` to
  `docker-compose.dev.yml` so migrations can run without the
  `docker run --network` workaround.
...
- [ ] `*.tsbuildinfo` ignored; `.env.example` corrected and committed
- [ ] `turbo.json` env passthrough complete; `dev:nest` script added
